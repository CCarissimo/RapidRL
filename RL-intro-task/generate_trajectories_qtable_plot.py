...
for row in range(len(env.grid[:,0])):
    for col in range(len(env.grid[0,:])):
        q_table[str(np.array([row, col]))] = {action: 10 for action in env.actions}
...
for i in tqdm.tqdm(range(n_episodes)):
    ...
    while not terminal:
        ...
        q_table[str(state)][action] = q_table[str(state)][action] + alpha*(reward+gamma*max(v for v in q_table[str(state)].values()) - q_table[str(state)][action])
        visits[str(state)] += 1
